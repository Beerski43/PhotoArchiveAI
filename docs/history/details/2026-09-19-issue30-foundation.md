...
## 5. レビューで見つかった、この作業自身の欠陥

PR #31 のレビューで6件の指摘を受けた。**うち2件は、この作業で新設した仕組みが
そのまま壊れていたもの。**

| # | 内容 |
|---|---|
| 1 | **`run_regression.sh` の集計行が、端末で実行すると必ず `0 passed / 0 failed` になっていた。** pytest は出力先が端末だと着色するので、シェルの `grep -E '^[0-9]+ passed'` が行頭のエスケープに阻まれて必ず外れる。しかも既定値の 0 が出るので**成功のように見えた**。PR 本文に貼った `107 passed` は、標準出力が端末でない経路で取れた値。集計を `scripts/summarize_pytest.py` に切り出し、着色を剥がしてから数えるようにした。**集計できないときは非ゼロで終了する**ので、0 が成功に見えることはもう無い |
| 2 | **`archive_worklog.py` が日付エントリ以外の `##` 節を黙って消していた。** 警告は出るが書き戻さず、切り出しが不要なときも WORKLOG.md を無条件に上書きするため、その場で消える。このスクリプトは毎コミット実行する決まりなので、日付以外の節を1つ書いた瞬間に git 管理下の文書が欠落する。最初の日付エントリより前の節は前書きとしてその場に残し、あとの節は末尾へ移すようにした |
| 3 | **`--allow-missing-embeddings` の逃げ道に、A3 が防ごうとした状態が残っていた。** `detector_version` を無条件に書いていたため、モデルを設置しても差分判定が全件スキップし、`--force-rescan`（手動割り当てを巻き添えにする）以外に回収手段が無かった。特徴量のモデルが無いまま顔を貯めた場合は版を書かず、次の通常 `scan` が自動で拾い直すようにした |
| 4 | ROADMAP の「現在地」がマージした瞬間に嘘になる（`#30 で実施中` の Phase が2つ残る） |
| 5 | `config.REPO_ROOT` が editable install 前提。**判定を `REPO_ROOT` 自身で書いたら自分のテストに落とされた**（通常インストールでは `REPO_ROOT` が `lib/python3.x` を指し、`site-packages` を含まない）。モジュールの位置で判断する形に直した |
| 6 | `.gitignore` の `config/rule.json` が名指しで、#27 で YAML にすると漏れる。`config/*` + `!config/*.sample.json` にした |

1・2・3・5 にはそれぞれ回帰テストを足した（`test_pytest_summary.py` 9件、
`test_worklog_archive.py` に6件、`test_scanner_incremental.py` に2件、
`test_config.py` に3件）。
