...
**実測の道具（`photoarchive evaluate`）は用意した。** 残るのは実データの作業
（全件スキャンと手動割り当て）と、その結果を見ての閾値の確定。
...
| Phase 4 GUI の作り込み | **一部着手** | プレビューに撮影日時とフォルダを出す（#41。Phase 3 の手順2を止めていたため先行）。残りは人物ごとの枚数表示、顔候補の推薦ほか | #18 |
