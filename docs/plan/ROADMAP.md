...
（#41・#48・#52 は PR #51 まで、#53 は PR #54 でマージ済み。**#55 が起票中の PR**）。
...
| Phase 4 GUI の作り込み | **前倒しで着手** | #41・#48・#52・#53 はマージ済み、**#55（フォルダ単位の一括除外）を実装中**。**Phase 3 の手順2が詰まるため、残りも前倒しする**（判断の数を減らす方向。顔候補の推薦は作らない） | #18 |
...
| 候補 | 状態 | 測定した根拠（2026-09-20） |
|---|---|---|
| フォルダ単位の一括除外 | **#55 で実装中** | 上位100フォルダで 51.5%（30,094件）。最大は結婚式1,357件で、ほぼ他人 |
| 1枚1顔をまとめて処理 | 未着手 | 21,961枚（38%）は写っている顔が1つだけ |
| キーボードで割り当て | 未着手 | 自動化が効かない以上、1件あたりの手数がそのまま総時間 |
...
**どの候補から入るかは、2026-09-20 にユーザーが「フォルダ単位の一括除外」を
選んだ**（子 Issue #55）。残る2つも同じ考え方で、順に子 Issue へ切る。
